...
def cut_metis_fov(hdu, rotate=False):
    ...
    fov2 = hdu.header['OUT_FOV']*3600/hdu.header['CDELT2'] - 20 # pix- 
...
def cart_to_polar_v2(img_arr, r_arr, phi_arr, xc, yc, rsun_pix, rot_angle=None, cval=0.0):
    """
    Convert image array from Cartesian (`x`,`y`) to polar (`Rsun`, `phi`) 
...
def cart_to_polar(img_fits, dr = 1, start_angle = 0, end_angle = 360, dphi = 1, rot_angle=None, cval=0.0, cut_out_fov = 0, cut_in_fov = 0):
    """
    Convert image array from Cartesian (`x`,`y`) to polar (`Rsun`, `phi`) 
    coordinates by means of CylindricalRepresentation object of 
    astropy.coordinates.representation library.
e  ----------
    ...
    rsun_pix = img_fits.header['RSUN_ARC']/img_fits.header['CDELT1'] 

    xc = img_fits.header['SUN_XCEN']
    yc = img_fits.header['SUN_YCEN']
    
    in_fov = img_fits.header['INN_FOV']*3600/img_fits.header['RSUN_ARC'] * rsun_pix + cut_in_fov # [px]
    out_fov = img_fits.header['OUT_FOV']*3600/img_fits.header['RSUN_ARC'] * rsun_pix - cut_out_fov # [px]
    
    r_arr = np.linspace(in_fov, out_fov, round((out_fov - in_fov) / dr), endpoint = False)
    
    phi_arr = np.linspace(start_angle, end_angle, round((end_angle - start_angle) / dphi))

    img_arr = img_fits.data
    
    if rot_angle is not None:
        phi_arr = phi_arr - rot_angle
        
    phi_matrix, r_matrix = np.meshgrid(phi_arr, r_arr)
    polar_repr = CylindricalRepresentation(r_matrix, phi_matrix*u.deg, np.zeros(r_matrix.shape))
    cart_repr = polar_repr.to_cartesian()
    polar_inds = np.array([cart_repr.y + yc, cart_repr.x + xc])  #  NB: index sequence not like in IDL: img_arr[x,y]
    polar_img_arr = ndimage.map_coordinates(img_arr, polar_inds, order=1, cval=cval)

    # r_matrix[np.isnan(polar_img_arr)] = np.nan
    
    return polar_img_arr, r_matrix, r_arr, rsun_pix, phi_arr
...
def cut_img_arr_fov(img_arr, xc, yc, rsun_pix, r1, r2, cval=np.nan):
    ...
    yc : `float`
        The second Cartesian coordinate (along `y`-axis) of the Sun center.
    rsun_pix : `float`
        The radius of the Sun in pixels of ``img_arr``.
    r1, r2 : `float`
...
def align_polar_maps(polar_A, r_matrix_A, r_arr_A, polar_B, r_matrix_B, r_arr_B):
    """
    Resamples A and B polar maps onto a common radial grid.

    This function determines the total radial extent of both maps, creates a
    new high-resolution grid, and uses linear interpolation to resample
    both maps onto this common grid.

    Args:
        polar_A (np.ndarray): The 2D polar-transformed A data array.
        r_arr_A (np.ndarray): The 1D array of radial coordinates for the A map.
        polar_B (np.ndarray): The 2D polar-transformed B data array.
        r_arr_B (np.ndarray): The 1D array of radial coordinates for the B map.

    Returns:
        tuple: A tuple containing:
            - r_common (np.ndarray): The new common 1D radial coordinate array.
            - polar_A_resampled (np.ndarray): The resampled A map.
            - polar_B_resampled (np.ndarray): The resampled B map.
    """
    print("--- Aligning Polar Maps ---")

    # Define the new common radial grid.
    # It will span from the lowest to the highest radial value across both maps.
    # We'll use the higher of the two resolutions (i.e., the larger number of rows)
    
    r_min_common = max(r_arr_A.min(), r_arr_B.min())
    r_max_common = min(r_arr_A.max(), r_arr_B.max())
    dr = min(np.diff(r_arr_A)[0], np.diff(r_arr_B)[0])
    num_steps_common = round((r_max_common - r_min_common) / dr)
    try:
        r_common = np.linspace(r_min_common, r_max_common, num_steps_common)
    except ValueError:
        raise ValueError('Provided polar images do not overlap.')
        
    r_matrix_common = np.tile(r_common.reshape(-1, 1), (1, polar_B.shape[1]))
    
    r_min_unite = min(r_arr_A.min(), r_arr_B.min())
    r_max_unite = max(r_arr_A.max(), r_arr_B.max())
    num_steps_unite = round((r_max_unite - r_min_unite) / dr)
    r_unite = np.linspace(r_min_unite, r_max_unite, num_steps_unite)

    r_unite = np.unique(np.sort(np.concatenate((r_unite, np.array([ r_arr_A[0], r_arr_A[-1], r_arr_B[0], r_arr_B[-1] ])))))

    r_unite_flip = np.flip(r_unite) 

    r_matrix_unite = np.tile(r_unite.reshape(-1, 1), (1, polar_B.shape[1]))

    mask_B = ~np.isnan(polar_B) # True where there is not a NaN
    mask_A = ~np.isnan(polar_A)
  
    mask_A_resampled = np.zeros((len(r_common), polar_B.shape[1]), dtype = bool)
    mask_B_resampled = np.zeros((len(r_common), polar_B.shape[1]), dtype = bool)

    for i, angle in enumerate(np.arange(polar_B.shape[1])): # Assuming that both the A and Ne maps have the same angular resolution (tipically = 1 deg)
        
        r_col_B = r_matrix_B[mask_B[:, angle], angle]
        r_col_A = r_matrix_A[mask_A[:, angle], angle]
    
        B_col = mask_B[mask_B[:, angle], angle] 
        A_col = mask_A[mask_A[:, angle], angle]

        if len(r_col_A) == 0 or len(r_col_B) == 0 or len(A_col) == 0 or len(B_col) == 0:

            mask_A_resampled[:, angle] = False
            mask_B_resampled[:, angle] = False

        else:
            interp_mask_A = interp1d(r_col_A, A_col, axis=0, kind='nearest', 
                                 bounds_error=False, fill_value=np.nan)
            
            interp_mask_B = interp1d(r_col_B, B_col, axis=0, kind='nearest', 
                                 bounds_error=False, fill_value=np.nan)
    
            
            mask_A_resampled[:, angle] = interp_mask_A(r_common) > 0.5
            mask_B_resampled[:, angle] = interp_mask_B(r_common) > 0.5

    mask_common = mask_A_resampled & mask_B_resampled
    
    start_idx = np.abs(r_arr_A[-1] - r_unite_flip).argmin()
    end_idx = np.abs(r_arr_B[0] - r_unite_flip).argmin()
 
    polar_A_resampled = np.zeros((len(r_common), polar_B.shape[1]))
    polar_B_resampled = np.zeros((len(r_common), polar_B.shape[1]))
    ...
    for i, angle in enumerate(np.arange(polar_B.shape[1])): # Assuming that both the A and Ne maps have the same angular resolution (tipically = 1 deg)
        ...
        B_col = polar_B[mask_B[:, angle], angle] 
        A_col = polar_A[mask_A[:, angle], angle]


        
        if len(r_col_A) == 0 or len(r_col_B) == 0 or len(A_col) == 0 or len(B_col) == 0:

            polar_A_resampled[:, angle] = False
            polar_B_resampled[:, angle] = False

        else: 
            interp_A = interp1d(r_col_A, A_col, axis=0, kind='linear', 
                                 bounds_error=False, fill_value=np.nan)
            
            interp_B = interp1d(r_col_B, B_col, axis=0, kind='linear', 
                                 bounds_error=False, fill_value=np.nan)
    
            idxs = mask_common[:, angle]  # integer row indices
            
            polar_A_resampled[idxs, angle] = interp_A(r_common[idxs])
            polar_B_resampled[idxs, angle] = interp_B(r_common[idxs])

    polar_A_resampled = np.where(polar_A_resampled == 0, np.nan, polar_A_resampled)
    polar_B_resampled = np.where(polar_B_resampled == 0, np.nan, polar_B_resampled) 

    r_matrix_common = np.tile(r_common.reshape(-1, 1), (1, polar_B.shape[1]))
    r_matrix_common = np.where(mask_common, r_matrix_common, np.nan)
        
        
    print(f"Original A shape: {polar_A.shape}, Original B shape: {polar_B.shape}")
    print(f"Resampled A shape: {polar_A_resampled.shape}, Resampled B shape: {polar_B_resampled.shape}")
    print(f"New common grid created with {num_steps_common} radial steps from {r_min_common:.2f} R☉ to {r_max_common:.2f} R☉.")
    print("--- Alignment Complete ---")
    
    return polar_A_resampled, polar_B_resampled, r_matrix_common, r_common


def fit_negative_power_series(r_arr, radial_ne, degree=3):
    r_arr = np.asarray(r_arr)
    radial_ne = np.asarray(radial_ne)

    # Build the design matrix for terms [1/r, 1/r^2, ...]
    X = np.column_stack([r_arr**(-k) for k in range(1, degree+1)])
    
    # Solve least squares: coefficients for negative powers
    coeffs, _, _, _ = np.linalg.lstsq(X, radial_ne, rcond=None)

    def model(r):
        r = np.asarray(r)
        X_pred = np.column_stack([r**(-k) for k in range(1, degree+1)])
        y_pred = X_pred @ coeffs
        return np.clip(y_pred, 0, None)  # No negatives
    
    return model, coeffs

def sqrt_model(r, a, b):
    return a + b * np.sqrt(r)
